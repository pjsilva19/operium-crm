-- =====================================================
-- MIGRACIÓN COMPLETA VIAJES - Eliminar antigua y crear nueva
-- =====================================================
-- IMPORTANTE: Esto eliminará todos los datos existentes en viajes
-- Si necesitas preservar datos, primero haz un backup

-- Paso 1: Eliminar tabla antigua y todas sus dependencias
DROP TABLE IF EXISTS public.viajes CASCADE;

-- Paso 2: Crear tabla nueva con estructura completa
...
CREATE INDEX idx_viajes_fecha_carga ON public.viajes(fecha_carga);
CREATE INDEX idx_viajes_cliente_id ON public.viajes(cliente_id);
CREATE INDEX idx_viajes_transportista_id ON public.viajes(transportista_id);
CREATE INDEX idx_viajes_sucursal_id ON public.viajes(sucursal_id);
CREATE INDEX idx_viajes_is_deleted ON public.viajes(is_deleted);
CREATE INDEX idx_viajes_created_at ON public.viajes(created_at DESC);

-- Paso 4: Trigger para actualizar updated_at automáticamente
...
-- Nota: Solo permitimos actualizar is_deleted a true (soft delete)
